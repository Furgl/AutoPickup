...
public class CommandBlacklist extends CommandBase
{
	...
	public CommandBlacklist() {
		...
		items.add(Item.getItemFromBlock(Blocks.field_189877_df).getItemStackDisplayName(new ItemStack(Blocks.field_189877_df)).replace(" ", "_"));
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		Config.syncFromConfig(sender.getName());
		if (args.length == 0) {
			sender.addChatMessage(new TextComponentString("[AutoPickup] Blacklist Contains:").setStyle(new Style().setColor(COLOR_NEUTRAL_DARK)));
			for (int i=0; i < Config.blacklistNames.size(); i++)
				sender.addChatMessage(new TextComponentString("- "+Config.blacklistNames.get(i)).setStyle(new Style().setColor(COLOR_NEUTRAL_LIGHT)));
			return;
		}
		else if (args.length == 1 && args[0].equalsIgnoreCase("clear"))	{
			Config.blacklistNames.clear();
			Config.syncToConfig(sender.getName());
			sender.addChatMessage(new TextComponentString("[AutoPickup] Blacklist cleared.").setStyle(new Style().setColor(COLOR_SUCCESS)));
			return;
		}
		//AUTOADD
		else if (args.length == 1 && args[0].equalsIgnoreCase("autoAdd")) {
			sender.addChatMessage(new TextComponentString("[AutoPickup] AutoAdd is currently "+(Config.autoAdd ? "enabled." : "disabled.")).setStyle(new Style().setColor(COLOR_NEUTRAL_LIGHT)));
			return;
		}
			...
			if (args[1].equalsIgnoreCase("enable")) {
				Config.autoAdd = true;
				Config.syncToConfig(sender.getName());
				sender.addChatMessage(new TextComponentString("[AutoPickup] AutoAdd enabled.").setStyle(new Style().setColor(COLOR_SUCCESS)));
				return;
			}
			else if (args[1].equalsIgnoreCase("disable")) {
				Config.autoAdd = false;
				Config.syncToConfig(sender.getName());
				sender.addChatMessage(new TextComponentString("[AutoPickup] AutoAdd disabled.").setStyle(new Style().setColor(COLOR_SUCCESS)));
				return;
			}
			else {
				sender.addChatMessage(new TextComponentString("Usage: /b autoAdd [enable/disable]").setStyle(new Style().setColor(COLOR_FAIL)));
				return;
			}
		}
		//PRESETS
		else if (args.length == 1 && args[0].equalsIgnoreCase("preset")) {
			sender.addChatMessage(new TextComponentString("[AutoPickup] A preset is a collection of items that can be added to or removed from the blacklist easily.").setStyle(new Style().setColor(COLOR_NEUTRAL_LIGHT)));
			return;
		}
		else if (args.length == 2 && args[0].equalsIgnoreCase("preset")) {
			for (int i=0; i<presets.size(); i++) {
				if (args[1].equalsIgnoreCase(presets.get(i))) {
					sender.addChatMessage(new TextComponentString("[AutoPickup] The "+presets.get(i)+" preset contains these items:").setStyle(new Style().setColor(COLOR_NEUTRAL_DARK)));
					for (String item : presetItems.get(i))
						sender.addChatMessage(new TextComponentString("- "+item).setStyle(new Style().setColor(COLOR_NEUTRAL_LIGHT)));
					return;
				}
			}
			sender.addChatMessage(new TextComponentString("Usage: /b preset [preset] [enable/disable]").setStyle(new Style().setColor(COLOR_FAIL)));
			return;
		}
		else if (args.length == 3 && args[0].equalsIgnoreCase("preset")) {
			for (int i=0; i<presets.size(); i++) {
				if (args[1].equalsIgnoreCase(presets.get(i))) {
					...
					if (args[2].equalsIgnoreCase("enable")) {
						...
						Config.syncToConfig(sender.getName());
						sender.addChatMessage(new TextComponentString("[AutoPickup] Added "+changedItems+" items to blacklist.").setStyle(new Style().setColor(COLOR_SUCCESS)));
						return;
					}
						...
						Config.syncToConfig(sender.getName());
						sender.addChatMessage(new TextComponentString("[AutoPickup] Removed "+changedItems+" items from blacklist.").setStyle(new Style().setColor(COLOR_SUCCESS)));
						return;
					}
				}
			}
		...
		//ADD/REMOVE
			...
			if (args[0].equalsIgnoreCase("add")) {													
				if (Config.blacklistNames.contains(args[1])) {
					sender.addChatMessage(new TextComponentString("[AutoPickup] Blacklist already contains "+args[1]+".").setStyle(new Style().setColor(COLOR_FAIL)));
					return;	
				}			
				else if (getDisplayNames().contains(args[1])) {
					Config.blacklistNames.add(args[1]);
					Config.syncToConfig(sender.getName());
					sender.addChatMessage(new TextComponentString("[AutoPickup] Added "+args[1]+" to blacklist.").setStyle(new Style().setColor(COLOR_SUCCESS)));
					return;
				}
			}
			else if (args[0].equalsIgnoreCase("remove")) {
				if (Config.blacklistNames.contains(args[1])) {
					...
					Config.syncToConfig(sender.getName());
					sender.addChatMessage(new TextComponentString("[AutoPickup] Removed "+args[1]+" from blacklist.").setStyle(new Style().setColor(COLOR_SUCCESS)));
					return;
				}
				else {
					sender.addChatMessage(new TextComponentString("[AutoPickup] Blacklist does not contain "+args[1]+".").setStyle(new Style().setColor(COLOR_FAIL)));
					return;
				}
			}
			else {
				sender.addChatMessage(new TextComponentString("Usage: /b <action> [<item>]").setStyle(new Style().setColor(COLOR_FAIL)));
				return;
			}
		}
		sender.addChatMessage(new TextComponentString("Usage: /b <action> [<item/true/false>]").setStyle(new Style().setColor(COLOR_FAIL)));
	}
